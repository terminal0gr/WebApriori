/*----------------------------------------------------------------------
  ...
  History : 2008.08.18 item set reporter created in tract.[ch]
            ...
            2019.04.15 function isr_addncw() added (no perf. ext. check)
            2019.05.12 macros for stdout and stderr added (for fim4r)
----------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#ifdef ISR_NOSTDFILE            /* to prevent warnings */
#define ISR_STDOUT    NULL      /* when compiling fim4r */
#define ISR_STDERR    NULL
#else
#define ISR_STDOUT    stdout
#define ISR_STDERR    stderr
#endif
...
{                               /* --- open an output file */
  ...
  if (file)                     /* if a file is given, */
    ...
    file = ISR_STDOUT; rep->name = "<stdout>"; }
...
{                               /* --- close the output file */
  ...
  r |= ((rep->file == ISR_STDOUT) || (rep->file == ISR_STDERR))
...
{                               /* --- set/open trans. id output file */
  ...
  if (file) {                   /* if a file is given directly, */
    ...
    else if (file == ISR_STDOUT) rep->tidname = "<stdout>";
    else if (file == ISR_STDERR) rep->tidname = "<stderr>";
    else                         rep->tidname = "<unknown>"; }
  else if (! name) {            /* if no name is given */
    file = NULL;                 rep->tidname = "<null>"; }
  else if (!*name) {            /* if an empty name is given */
    file = ISR_STDOUT;           rep->tidname = "<stdout>"; }
  else {                        /* if a proper name is given */
...
{                               /* --- close trans. id output file */
  ...
  r |= ((rep->tidfile == ISR_STDOUT) || (rep->tidfile == ISR_STDERR))
...
int isr_addncw (ISREPORT *rep, ITEM item, RSUPP supp, double wgt)
{                               /* --- add an item (support & weight) */
  assert(rep && (item >= 0)     /* check the function arguments */
  &&    (item < ib_cnt(rep->base)));
  /* assert(!isr_uses(rep, item)); */
...
/* In contrast to isr_addwgt(), the function isr_addncw() does not    */
/* check whether the extended prefix possesses a perfect extension.   */
