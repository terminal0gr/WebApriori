...
# Malliaridis 30/06/2025. Memory consumtion
import psutil
import os
...
data    static recursion/output data as a list
    ...
    for k in range(len(tadb)):  # traverse the items/item sets
        ...
        xset = iset +[i]        # add the current item to the set
...
returns if a parameter 'out' is a list, the return value is a list of
    ...
    # Malliaridis 30/06/2025. Memory consumtion
    currentMemory = process.memory_full_info().uss
    if maxMemoryUSS<currentMemory:
        maxMemoryUSS=currentMemory
...
if __name__ == '__main__':
    ...
    with open(fixed[0], 'rt', encoding='utf-8-sig') as inp:
    ...
    t1 = time()                  # mine frequent item sets
    stderr.write('mining frequent item sets ... ')
    if len(fixed) < 2:          # if no output file is given
        n = eclat(tracts, **opts) # only count frequent item sets
    ...
    # Malliaridis 30/06/2025. Memory consumtion & total time
    stderr.write('Total Execution Time: [%.2fs].\n' % (time()-t))
    maxMemoryUSS/=(1024*1024)
    stderr.write('          Max Memory: [%.0fMB].\n' % maxMemoryUSS)
