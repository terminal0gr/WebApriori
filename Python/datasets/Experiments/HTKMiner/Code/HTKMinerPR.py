...
def process_chunk(chunk, delimiter, shared_item_dict, shared_item_dict_reversed):
    vR_chunk = {}
    iSet_chunk = set()
    ...
    for line in chunk:
        ...
        for item in line.strip().split(sep=delimiter):
            if item not in iSet_chunk:
                item_index_chunk += 1
                shared_item_dict[item_index_chunk] = item
                shared_item_dict_reversed[item] = item_index_chunk
                vR_chunk[(item_index_chunk,)] = [trans_index_chunk]
                iSet_chunk.add(item)
            else:
                vR_chunk[(shared_item_dict_reversed[item],)].append(trans_index_chunk)
    
    return vR_chunk, shared_item_dict, shared_item_dict_reversed

def read_file_parallel(dataset_file, delimiter, num_workers=4):
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        results = executor.map(
            ...
            [shared_item_dict] * len(chunks),
            [shared_item_dict_reversed] * len(chunks),
        )

    # Combine results from each process
    combined_vR = {}
    for vR_chunk, item_dict, item_dict_reversed in results:
        combined_vR.update(vR_chunk)
        shared_item_dict.update(item_dict)
        shared_item_dict_reversed.update(item_dict_reversed)

    return combined_vR, shared_item_dict
...
class HTKMiner: 
    ...
    delimiter = ','  # Set your delimiter here
    vR, item_dict = read_file_parallel(dataset_file, delimiter)

    def readDatasetFile(self):

        vR, self.item_dict = read_file_parallel(self.dataset_file, self.delimiter)
