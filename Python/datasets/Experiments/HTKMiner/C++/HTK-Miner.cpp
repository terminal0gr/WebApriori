...
// ==========================================
class HTKMiner {
    // To store actual findings: Itemset (vector of ints) -> Support
    std::vector<FoundItemset> finalTopK; // This holds the actual results

    std::unordered_map<int, std::vector<int>> vR; // stores original data in its vertical representation

    QuickHeap heap; // Placeholder, will be initialized properly in readDatasetFile
    ...
    bool tidSet=true; // true = intersection, false = diffset

    std::string delimiter;

public:
    HTKMiner(int k, std::string delim = " ", bool tidSetMode = true, bool bitSetMode = true) 
        : heap(k), delimiter(delim), tidSet(tidSetMode), bitSet(bitSetMode), topK(k) {}

    void mine(const std::string& filepath) {
        auto start = std::chrono::high_resolution_clock::now();

        // Level 1
        std::vector<FoundItemset> initialTopK = readDatasetFile(filepath);
        std::cout << "Level 1 items: " << initialTopK.size() << "\n";

        mineNextLevelIntersectionBitSet(initialTopK);
        ...
        int level = 1;
        while (!initialTopK.empty()) {
            std::vector<Candidate> nextLevel;
            int n = currentLevel.size();
            int minCount = heap.getMinSup();

            for (int i = 0; i < n; ++i) {
                if (currentLevel[i].support < minCount) continue;

                for (int j = i + 1; j < n; ++j) {
                    if (currentLevel[j].support < minCount) continue;
...
private:
    std::vector<FoundItemset> readDatasetFile(const std::string& path) {
