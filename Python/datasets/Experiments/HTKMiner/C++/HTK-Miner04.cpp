...
private:
    std::vector<Candidate> readDatasetFile(const std::string& path) {
        ...
        auto [item1TopK, calculated_min_count] = InitialTopKFI(vR, cutoff_support);
        
        // Update Class Global State
    ...
    std::pair<std::vector<ItemStat>, int> InitialTopKFI(const VerticalDB& vR, int& min_count_out) {
        ...
        std::vector<ItemStat> item1TopK;
// Pre-allocate space for at least K items to minimize early reallocations
item1TopK.reserve(topK); 

int mS = -1;
for (size_t i = 0; i < stats.size(); ++i) {
    // Logic: If we've reached K items, check if the next support value changes
    if (i >= (size_t)topK - 1) {
        if (mS == -1) {
            mS = stats[i].first;
        } else if (stats[i].first != mS) {
            break; // We found an item with lower support than the K-th item; Stop.
        }
    }
    item1TopK.push_back(stats[i]);
}
