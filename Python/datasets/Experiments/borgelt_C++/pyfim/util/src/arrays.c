/*----------------------------------------------------------------------
  ...
  History : 1996.09.16 file created as arrays.c
            ...
            2018.06.18 bug in function idx_qsort/_qrec fixed
            2018.07.19 quantile functions for index arrays added
            2018.08.26 bisection and binary search functions for indexes
            2018.09.01 bisection yields index after last equal element
            2019.03.29 bug in bisections functions fixed
...
{                               /* --- recursive part of sort */       \
  ...
  type   p, t;                  /* pivot element and exchange buffer */\
  ...
  do {                          /* sections sort loop */               \
    ...
    if      (p < *l) p = *l;    /* compute median of three */          \
    else if (p > *r) p = *r;    /* to find a better pivot */           \
    while (1) {                 /* split and exchange loop */          \
      while (*++l < p);         /* skip smaller elems. on the left */  \
      while (*--r > p);         /* skip greater elems. on the right */ \
...
{                               /* --- let element sift down in heap */\
  size_t i;                     /* index of successor in heap */       \
  ...
  for (l = 0, r = n; l < r; ) { /* while search range is not empty */  \
    ...
    if      (key > t) l = m+1;  /* to the middle element and */        \
      ...
      while (++m < r)           /* if found, skip equal elements */    \
        if (key < array[m]) break;                                     \
      return m-1;               /* if match found, return index */     \
...
#define QUANTILE(name, type) \
type name##_quantile (type *array, size_t n, size_t k) \
{                               /* --- find k-th smallest element */   \
  type *q;                      /* location of quantile element */     \
  type *l, *r;                  /* pointers to exchange positions */   \
  type p, t;                    /* pivot element and exchange buffer */\
                                                                       \
  q = array +k;                 /* get quantile location */            \
  while (n > 1) {               /* sections split loop */              \
    l = array; r = array +n -1; /* start at left and right boundary */ \
    if (*l > *r) {              /* get the first and last elements */  \
      t = *l; *l = *r; *r = t; }/* and bring them into right order */  \
    p = array[n/2];             /* get the middle element as pivot */  \
    if      (p < *l) p = *l;    /* compute median of three */          \
    else if (p > *r) p = *r;    /* to find a better pivot */           \
    while (1) {                 /* split and exchange loop */          \
      while (*++l < p);         /* skip smaller elems. on the left */  \
      while (*--r > p);         /* skip greater elems. on the right */ \
      if (l >= r) break;        /* if <= one element left, abort */    \
      t = *l; *l = *r; *r = t;  /* otherwise exchange elements */      \
    }                                                                  \
    if (l <= r) {               /* if exactly one element left */      \
      if (l == q) return *q;    /* if at quantile, return quantile, */ \
      l++; r--;                 /* otherwise skip the pivot element */ \
    }                                                                  \
    if (q <= r)                 /* if quantile in left/lower section */\
      n = 1 +(size_t)(r-array); /* compute the new section size */     \
    else {                      /* if quantile in right/up. section */ \
      n = n -(size_t)(l-array); /* compute the new section size */     \
      array = l;                /* section now starts with l */        \
    }                           /* (skip the left section) */          \
  }                                                                    \
  return *q;                    /* return the quantile element */      \
}  /* qtile() */
...
{                               /* --- recursive part of quicksort */
  ...
  void   *p, *t;                /* pivot element and exchange buffer */
  ...
  do {                          /* sections sort loop */
    ...
    p = a[n/2];                 /* get the middle element as pivot */
    if      (cmp(p, *l, data) < 0) p = *l;  /* try to find a */
    else if (cmp(p, *r, data) > 0) p = *r;  /* better pivot */
    while (1) {                 /* split and exchange loop */
      while (cmp(*++l, p, data) < 0)      /* skip left  elements that */
        ;                       /* are smaller than the pivot element */
      while (cmp(*--r, p, data) > 0)      /* skip right elements that */
...
{                               /* --- do a binary search */
  ...
  for (l = 0, r = n; l < r; ) { /* while search range is not empty */
    ...
    if      (c > 0) l = m+1;    /* adapt the search range */
      ...
      while (++m < r)           /* skip equal elements */
        if (cmp(key, a[m], data) < 0) break;
      return m-1;               /* then return index */
    }                           /* after last equal element */
  }
  return l;                     /* return the insertion position */
}  /* ptr_bisect() */
...
void* ptr_quantile (void *array, size_t n, size_t k,
                    CMPFN *cmp, void *data)
{                               /* --- find k-th smallest element */
  void **q;                     /* location of quantile element */
  void **l, **r;                /* pointers to exchange positions */
  void *p, *t;                  /* pivot element and exchange buffer */
  void **a = (void**)array;     /* typed array */

  q = a +k;                     /* get quantile location */
  while (n > 1) {               /* sections split loop */
    ...
    while (1) {                 /* split and exchange loop */
      ...
      while (cmp(*--r, p, data) > 0)      /* skip right elements that */
        ;                       /* are greater than the pivot element */
      if (l >= r) break;        /* if at most one element left, abort */
      t = *l; *l = *r; *r = t;  /* otherwise exchange elements */
    }
    if (l <= r) {               /* if exactly one element left */
      if (l == q) return *q;    /* if at quantile, return quantile, */
    ...
    if (q <= r)                 /* if quantile in left/lower section */
      n = 1 +(size_t)(r-a);     /* compute the new section size */
    else {                      /* if quantile in right/upper section */
      n = n -(size_t)(l-a);     /* compute the new section size */
      a = l;                    /* section now starts with l */
    }                           /* (skip the left section) */
  }
  return *q;                    /* return the quantile element */
}  /* ptr_quantile() */
...
{                               /* --- recursive part of quicksort */
  ...
  size_t p[OBJSIZE],t[OBJSIZE]; /* pivot element and exchange buffer */
  size_t m;                     /* number of elements in 2nd section */
  ...
  do {                          /* sections sort loop */
    ...
    x = a +(n/2) *size;         /* get the middle element as pivot */
    if      (cmp(x, l, data) < 0) memcpy(p, l, size); /* try to find */
    else if (cmp(x, r, data) > 0) memcpy(p, r, size); /* better pivot */
    else                          memcpy(p, x, size); /* (median of 3)*/
    while (1) {                 /* split and exchange loop */
      while (cmp(l += size, p, data) < 0) /* skip left  elements that */
        ;                       /* are smaller than the pivot element */
      while (cmp(r -= size, p, data) > 0) /* skip right elements that */
        ;                       /* are greater than the pivot element */
      if (l >= r) {             /* if at most one element left */
        if (l > r) break;       /* if no element, abort the loop */
...
{                               /* --- do a binary search */
  ...
  for (l = 0, r = n; l < r; ) { /* while search range is not empty */
    ...
    if      (c > 0) l = m+1;    /* adapt the search range */
      ...
      while (++m < r)           /* skip equal elements */
        if (cmp(key, a+m*size, data) < 0) break;
      return m;                 /* then return index */
    }                           /* after last equal element */
  }
  return l;                     /* return the insertion position */
}  /* obj_bisect() */
...
void* obj_quantile (void *array, size_t n, size_t k, size_t size,
                    CMPFN *cmp, void *data)
{                               /* --- recursive part of quicksort */
  char   *q;                    /* location of quantile element */
  char   *l, *r;                /* pointers to exchange positions */
  size_t p[OBJSIZE],t[OBJSIZE]; /* pivot element and exchange buffer */
  char   *a = (char*)array, *x; /* typed array, buffer */

  q = a +k *size;               /* get quantile location */
  while (n > 1) {               /* sections split loop */
...
{                               /* --- recursive part of sort */       \
  ...
  size_t m;                     /* number of elements in sections */   \
  type   p;                     /* buffer for (pivot) array element */ \
                                                                       \
  do {                          /* sections sort loop */               \
    l = index;                  /* start at left */                    \
    r = e = index +n -1;        /* and right boundary */               \
    if (array[*l] > array[*r]){ /* get the first and last elements */  \
      t = *l; *l = *r; *r = t; }/* and bring them into right order */  \
    p = array[index[n/2]];      /* get the middle element as pivot */  \
    if      (p < array[*l]) p = array[*l]; /* compute median of 3 */   \
    else if (p > array[*r]) p = array[*r]; /* to find better pivot */  \
    while (1) {                 /* split and exchange loop */          \
...
#define IDX_QUANTILE(name,tidx,type)                                   \
type name##_quantile (tidx *index, size_t n, size_t k,                 \
                      const type *array)                               \
{                               /* --- recursive part of sort */       \
  tidx   *q;                    /* location of quantile element */     \
  tidx   *l, *r;                /* pointers to exchange positions */   \
  tidx   t;                     /* pivot element and exchange buffer */\
  type   p;                     /* buffer for (pivot) array element */ \
                                                                       \
  q = index +k;                 /* get quantile location */            \
  while (n > 1) {               /* sections split loop */              \
    ...
    if (l <= r) {               /* if exactly one element left */      \
      if (l == q) return array[*q];                                    \
      l++; r--;                 /* if at quantile, return quantile, */ \
    }                           /* otherwise skip the pivot element */ \
    if (q <= r)                 /* if quantile in left/lower section */\
      n = 1 +(size_t)(r-index); /* compute the new section size */     \
    else {                      /* if quantile in right/up. section */ \
      n = n -(size_t)(l-index); /* compute the new section size */     \
      index = l;                /* section now starts with l */        \
    }                           /* (skip the left section) */          \
  }                                                                    \
  return array[*q];             /* return the quantile element */      \
}  /* quantile() */

/*--------------------------------------------------------------------*/

IDX_QUANTILE(i2i, int,    int)
IDX_QUANTILE(i2l, int,    long)
IDX_QUANTILE(i2x, int,    diff_t)
IDX_QUANTILE(i2z, int,    size_t)
IDX_QUANTILE(i2f, int,    float)
IDX_QUANTILE(i2d, int,    double)

IDX_QUANTILE(l2i, long,   int)
IDX_QUANTILE(l2l, long,   long)
IDX_QUANTILE(l2x, long,   diff_t)
IDX_QUANTILE(l2z, long,   size_t)
IDX_QUANTILE(l2f, long,   float)
IDX_QUANTILE(l2d, long,   double)

IDX_QUANTILE(x2i, diff_t, int)
IDX_QUANTILE(x2l, diff_t, long)
IDX_QUANTILE(x2x, diff_t, diff_t)
IDX_QUANTILE(x2z, diff_t, size_t)
IDX_QUANTILE(x2f, diff_t, float)
IDX_QUANTILE(x2d, diff_t, double)

/*--------------------------------------------------------------------*/

#define IDX_BSEARCH(name,tidx,type)                                    \
diff_t name##_bsearch (type key, const tidx *index, size_t n,          \
                       const type *array)                              \
{                               /* --- do a binary search */           \
  size_t l, r, m;               /* array indices */                    \
  type   t;                     /* array element */                    \
  ...
  assert(index && array);       /* check the function arguments */     \
  for (l = 0, r = n; l < r; ) { /* while search range is not empty */  \
    t = array[index[m = (l+r)/2]];  /* compare the given key */        \
    if      (key > t) l = m+1;  /* to the middle element and */        \
    else if (key < t) r = m;    /* adapt the search range */           \
    else return (diff_t)m;      /* according to the result */          \
  }                             /* if match found, return index */     \
...
IDX_BSEARCH(i2i, int,    int)
IDX_BSEARCH(i2l, int,    long)
IDX_BSEARCH(i2x, int,    diff_t)
IDX_BSEARCH(i2z, int,    size_t)
IDX_BSEARCH(i2f, int,    float)
IDX_BSEARCH(i2d, int,    double)

IDX_BSEARCH(l2i, long,   int)
IDX_BSEARCH(l2l, long,   long)
IDX_BSEARCH(l2x, long,   diff_t)
IDX_BSEARCH(l2z, long,   size_t)
IDX_BSEARCH(l2f, long,   float)
IDX_BSEARCH(l2d, long,   double)

IDX_BSEARCH(x2i, diff_t, int)
IDX_BSEARCH(x2l, diff_t, long)
IDX_BSEARCH(x2x, diff_t, diff_t)
IDX_BSEARCH(x2z, diff_t, size_t)
IDX_BSEARCH(x2f, diff_t, float)
IDX_BSEARCH(x2d, diff_t, double)

/*--------------------------------------------------------------------*/

#define IDX_BISECT(name,tidx,type)                                     \
size_t name##_bisect (type key, const tidx *index, size_t n,           \
                      const type *array)                               \
{                               /* --- do a bisection search */        \
  size_t l, r, m;               /* array indices */                    \
  type   t;                     /* array element */                    \
...
IDX_BISECT(i2i, int,    int)
IDX_BISECT(i2l, int,    long)
IDX_BISECT(i2x, int,    diff_t)
IDX_BISECT(i2z, int,    size_t)
IDX_BISECT(i2f, int,    float)
IDX_BISECT(i2d, int,    double)

IDX_BISECT(l2i, long,   int)
IDX_BISECT(l2l, long,   long)
IDX_BISECT(l2x, long,   diff_t)
IDX_BISECT(l2z, long,   size_t)
IDX_BISECT(l2f, long,   float)
IDX_BISECT(l2d, long,   double)

IDX_BISECT(x2i, diff_t, int)
IDX_BISECT(x2l, diff_t, long)
IDX_BISECT(x2x, diff_t, diff_t)
IDX_BISECT(x2z, diff_t, size_t)
IDX_BISECT(x2f, diff_t, float)
IDX_BISECT(x2d, diff_t, double)
...
{                               /* --- recursive part of quicksort */  \
  ...
  tidx   t;                     /* pivot element and exchange buffer */\
  size_t m;                     /* number of elements in 2nd section */\
  const void *p;                /* buffer for (pivot) array element */ \
                                                                       \
  do {                          /* sections sort loop */               \
    l = index;                  /* start at left */                    \
    r = e = index +n -1;        /* and right boundary */               \
    if (cmp(array[*l], array[*r], data) > 0) {                         \
      t = *l; *l = *r; *r = t;} /* arrange first and last element */   \
    p = array[index[n/2]];      /* compute median of 3 for pivot */    \
    if      (cmp(p, array[*l], data) < 0) p = array[*l];               \
    else if (cmp(p, array[*r], data) > 0) p = array[*r];               \
    while (1) {                 /* split and exchange loop */          \
...
{                               /* --- quick sort for index arrays */  \
  ...
  if (n < TH_INSERT)            /* if fewer elements than threshold */ \
    ...
    name##_qrec(index, n, a, cmp, data);     /* sort recursively */    \
    k = TH_INSERT -1;           /* and get the number of elements */   \
  }                             /* in the first array section */       \
  for (l = r = index; --k > 0;) /* find the smallest element */        \
    if (cmp(a[*++r], a[*l], data) < 0)                                 \
      l = r;                    /* among the first k elements */       \
  r = index;                    /* swap the smallest element */        \
  x = *l; *l = *r; *r = x;      /* to the front as a sentinel */       \
  for (i = n; --i > 0; ) {      /* standard insertion sort */          \
    t = a[x = *++r];            /* note the element to insert */       \
    for (l = r; cmp(a[*--l], t, data) > 0; )                           \
...
{                               /* --- heap sort for index arrays */   \
  ...
  r = n -1;                     /* of the array has heap structure */  \
  while (l > 0)                 /* while the heap is not complete, */  \
    name##_sift(index, --l, r, a, cmp, data);         /* extend it */  \
  while (1) {                   /* heap reduction loop */              \
    t        = index[0];        /* swap the greatest element */        \
    index[0] = index[r];        /* to the end of the array */          \
    index[r] = t;                                                      \
    if (--r <= 0) break;        /* if the heap is empty, abort */      \
    name##_sift(index, 0, r, a, cmp, data);                            \
...
#define I2P_BSEARCH(name,tidx)                                         \
diff_t name##_bsearch (const void *key, const tidx *index, size_t n,   \
                       const void *array, CMPFN *cmp, void *data)      \
{                               /* --- do a binary search */           \
  size_t l, r, m;               /* array indices */                    \
  const void *t;                /* array element */                    \
  const void **a = (const void**)array;                                \
                                                                       \
  assert(index && array && cmp);/* check the function arguments */     \
  for (l = 0, r = n; l < r; ) { /* while search range is not empty */  \
    t = a[m = (l+r)/2];         /* compare the given key */            \
    if      (cmp(key, t, data) < 0)                                    \
      l = m+1;                  /* to the middle element and */        \
    else if (cmp(key, t, data) > 0)                                    \
      r = m;                    /* adapt the search range */           \
    else return (diff_t)m;      /* according to the result */          \
  }                             /* if match found, return index */     \
  return (diff_t)-1;            /* return 'not found' */               \
}  /* bsearch() */
...
I2P_BSEARCH(i2p, int)
I2P_BSEARCH(l2p, long)
I2P_BSEARCH(x2p, diff_t)

/*--------------------------------------------------------------------*/

#define I2P_BISECT(name,tidx)                                          \
size_t name##_bisect (const void *key, const tidx *index, size_t n,    \
                      const void *array, CMPFN *cmp, void *data)       \
{                               /* --- do a binary search */           \
  size_t l, r, m;               /* array indices */                    \
  const void *t;                /* array element */                    \
